...
impl PrincipalCredentialRepository for PostgresPrincipalCredentialRepository<'_> {
    ...
    async fn list_by_principal(
        ...
        let rows = sqlx::query_as::<_, PrincipalCredentialRow>(
            "SELECT credential_id, principal_id, kind, lookup_key, secret_material, status, expires_at, last_used_at, created_at, revoked_at \
             FROM principal_credentials WHERE principal_id = ? ORDER BY created_at DESC LIMIT $1 OFFSET $2",
        )
    ...
    async fn count_by_principal(&mut self, principal_id: &str) -> Result<i64, DbError> {
        let n = sqlx::query_scalar::<_, i64>(
            "SELECT COUNT(*) FROM principal_credentials WHERE principal_id = $1",
        )
        .bind(principal_id)
        .fetch_one(&mut **self.tx)
        .await
