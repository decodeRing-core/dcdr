...
pub struct PostgresPluginConfigRepository<'a> {
    pub tx: &'a mut Transaction<'static, Postgres>,
}

impl PluginConfigRepository for PostgresPluginConfigRepository<'_> {
    async fn get_by_backend(
        ...
        let plugin_config: Option<PluginConfigRow> = sqlx::query_as::<_, PluginConfigRow>(
            "SELECT backend_name, secret_blob, updated_at FROM plugin_configs WHERE backend_name = $1 LIMIT 1",
        )
        .bind(backend_name)
